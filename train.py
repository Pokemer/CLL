...
parser.add_argument('--loss', type=str, default='CLL', help='SCE, CE, CLL')
...
def train_fixed(starting_epoch, data_loader, fixed_cnn, criterion, fixed_cnn_optmizer, fixed_cnn_scheduler):
    ...
    for epoch in tqdm(range(starting_epoch, args.epoch)):
        ...
        for images, labels in tqdm(data_loader["train_dataset"]):
            ...
            pred = fixed_cnn(images)
            if args.loss == 'CLL':
                loss = criterion(pred, labels, epoch)
            else:
...
def train():
    ...
    starting_epoch = 0
    train_fixed(starting_epoch, dataLoader, fixed_cnn, criterion, fixed_cnn_optmizer, fixed_cnn_scheduler)
